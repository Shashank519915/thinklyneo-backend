...
export async function verifyApiRequest(req: Request): Promise<VerifiedRequest | { error: string; status: number }> {
  ...
  if (isUnkeyConfigured) {
    try {
      ...
      if (!userId) {
        return { error: "API key is valid but has no owner configured", status: 401 };
      }

      // Add rate limit headers if present
      const rateLimitHeaders: Record<string, string> = {};
      if (result.ratelimit) {
        rateLimitHeaders["X-RateLimit-Limit"] = result.ratelimit.limit.toString();
        rateLimitHeaders["X-RateLimit-Remaining"] = result.ratelimit.remaining.toString();
        rateLimitHeaders["X-RateLimit-Reset"] = result.ratelimit.reset.toString();
      }
